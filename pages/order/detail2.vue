		...
		<div class="card-wrap mt20">
			<u-card :thumb-circle="true" :title-size="36" :full="true">
				...
				<view class="card-body" slot="body">
					...
					v-for="(goodInfo, index) in goodInfoArr" :key="goodInfo.id">
						<image :src="goodInfo.good_img" mode="aspectFill"></image>
						<view class="u-body-item-title u-line-2">
							<div class="item-title">{{goodInfo.good_name}}</div>
							<div class="item-price-wrap">
								<span class="item-price">{{goodInfo.good_price}}元</span>
								<span class="item-total">x{{goodInfo.good_count}}</span>
							</div>
						</view>
					</view>
				</view>
				<view class="card-foot" slot="foot">
					<span class="total">合计：<span>{{total_price}}元</span></span>
					<u-button type="error" size="medium" @click="showModal">提交订单</u-button>
				</view>
			</u-card>
		</div>
		<u-modal v-model="modalShow" title="付款详情" :content="total_price + '元'" 
...
<script>
	...
	export default {
		...
		methods: {
			// 获取商品信息
			async getGoodInfo(cart_detail_ids){
				const {data} = await this.$requestTool.get('/cart/queryByIds', {
					params: {
						ids: cart_detail_ids
					}
				});
				this.goodInfoArr = data;
				console.log(this.goodInfoArr);
				this.total_price = data.reduce((prev, next) => prev + next.good_price * next.good_count, 0);
			},
			// 获取管理员信息
			...
			setOrderParams(statu){
				const params = {
					...
					good_info: this.goodInfoArr.map((item) => {
						return {
		...
		onLoad(option) {
			const {cart_detail_ids, cart_id, admin_id} = option;
			this.getAddressInfo();
